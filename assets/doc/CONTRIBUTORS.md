...
- Provided patch for _strdate()_ in `ncidd.c` to fix null pointer in  
  _gettimeofday()_ in version 0.61.
...
- Helped rewrite _sendLog()_ code to eliminate corruption sending  
  large log files (greater than 2,000 lines).
- Converted `testclient` from using _netcat_ to using sockets.
...
- Created the `ncidd` configuration option _ignore1_ patches.
