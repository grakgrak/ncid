...
> you need to set _cidinput_ to 2 or 3 in **ncidd.conf**. Once you change **ncidd.conf**,
...
> SIP you need to set _cidinput_ to 2 or 3 in **ncidd.conf**. Once you
...
> only using YAC, or SIP and YAC, you need to set _cidinput_ to 2 or 3
...
> Because the POTS (Plain Old Telephone Service) lines are connected
> directly to the Whozz Calling device, you need to set _cidinput_ to
...
> The wc2ncid gateway script needs to be run at least once before using it with NCID. It also needs to be run whenever you change _wcaddr_ in **wc2ncid.conf**.
...
> only using Remote Notifier, set _cidinput_ to 2 or 3 in **ncidd.conf**.
> Once you change **ncidd.conf**, you must start/restart ncidd to read it.
...
> case you need to edit the _reject_ line at the end of the **rn2ncid.conf**
...
> _PageTo_ needs to be set to your mobile provider SMS e-mail address.
...
> then your _PageTo_ line becomes:
