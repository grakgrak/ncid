...
> message and send it to the NCID server. If the _notify_ option is used, it
...
>   _email2ncid.conf_, uncomment the line for variable NCIDSERVER and set
...
> > the _tohost_ and _toport_ variables in file **ncid2ncid.conf**, or by using
...
> > client. You specify sending servers using the _fromhostX_ and _fromportX_
...
> > see [Note 1](#gateways_note1) for an explanation of _cidinput_
...
> line to the OBi110 Telco Line connector. The OBi200 requires the _OBiLINE
> FXO to USB Phone Line Adapter_; connect the house Telco line to the OBiLINE.
...
> (Note: See [Note 1](#gateways_note1) for an explanation of _cidinput_)
...
> case you need to edit the _reject_ line at the end of **rn2ncid.conf**
...
> by your cell phone carrier.) The setting for _reject_ is usually of the
...
> - the **xdmf2ncid** **_lineid_** is the device name
> - see [Note 1](#gateways_note1) for an explanation of _cidinput_
...
> If using the Holtek HT9032D based PSTN Caller ID module, set **_ht9032 = 1_**.
...
> If you do not use a modem or serial device for Caller ID, you need to configure ncidd by
> changing this line in **ncidd.conf**:
...
> In **ncidd.conf** _cidinput_ specifies the Caller ID input:
