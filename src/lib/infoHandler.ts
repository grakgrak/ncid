import { ncidinfo, ncidStatusCache, type INcidRequest } from '$lib/ncidStores';
...
export class InfoHandler implements INcidRequest {
    ...
    getRequest(): string {
        ...
        if (tmp) {
            ...
            if (info) return `REQ: INFO ${info.NMBR}&&${info.NAME}\n`;
        }
        return '';
    }
    ...
    handler(line: string): { suppress: boolean; isFinished: boolean } {
        ...
        if (line.startsWith('411 ')) return { suppress: true, isFinished: true };
