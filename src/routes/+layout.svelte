<script lang="ts">
	...
	let VORON_PRINTER = $state<string>();
	let configPromise = $state(fetchAllConfig());
...
{#snippet header()}
	<h2 class="text-orange-500 m-3 w-44">Settings</h2>
{/snippet}

<Modal bind:showModal {header}>
	<div class="form-control">
		...
		<ul>
			...
			{:then config}
				{#each config as nv}
					<li>
						<button class="btn btn-sm my-1" onclick={() => edit(nv)}>{nv.name} = {nv.value}</button>
					</li>
				{/each}
			{/await}
		</ul>
...
<div class="w-full h-full flex flex-col overflow-hidden">
	<header class="font-bold rounded-lg m-2 bg-gray-300 h-11">
		<ul class="flex justify-around pt-2 text-gray-700 px-2">
			<li><Link href="/ncid">Caller ID</Link></li>
			...
			<li><Link target="__Rpi5__" href="https://192.168.1.100:4200">[Rpi 5]</Link></li>
			<!-- svelte-ignore a11y-click-events-have-key-events -->
			<!-- svelte-ignore a11y-no-static-element-interactions -->
			<button
				class="p-1 hover:bg-teal-500 hover:text-white rounded"
