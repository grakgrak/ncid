<script lang="ts">
    ...
    import { loginfo, ncidinfo, type Dictionary } from '$lib/store';
	import { onMount } from "svelte";
	import { ncidClient } from "$lib/ncidClient";
	import { WaitHandler } from "$lib/infoHandler";
	import EditButton from "$lib/components/EditButton.svelte";

    let client = ncidClient('ws://192.168.1.231:3334')
    ...
    const formatDateTime = (date: string, time: string) => `${date.slice(2,4)}-${date.slice(0,2)}-${date.slice(4)} ${time.slice(0,2)}:${time.slice(2)}`;
...
<div class="flex text-grey-300">
    <Vscroll width="w-1/2" height="h-[calc(100vh-80px)]">
        ...
        {#each $ncidinfo as data (data.ID)}
            <tr class="hover">
                <td class="pt-0 pb-0">
                    <EditButton client={client} data={data} />
                </td>
                <td class="text-xs pl-2">{formatDateTime(data.DATE, data.TIME)}</td>
                <td class="text-xs pl-2">{@html getLink(data)}</td>
                <td class:text-red-500={data.NAME === "NO NAME"} class="tooltip tooltip-bottom tooltip-accent text-xs pl-2" data-tip={tooltipData(data)}>{data.NAME}</td>
