<script lang="ts">
	...
	onMount(() => {
		const sse = new EventSource(
			'/mqtt?topic=alarm/sensor/%23&topic=alarm/status&topic=alarm/heartbeat'
		);
		...
		sse.onmessage = (e) => {
			...
			if (topic === 'alarm/sensor/pirDR') pirDR = msg;
			if (topic === 'alarm/sensor/pirLR') pirLR = msg;
			if (topic === 'alarm/sensor/pirHW') pirHW = msg;
			if (topic === 'alarm/sensor/door') pirDoor = msg;
			if (topic === 'alarm/status') alarmStatus = msg;
			if (topic === 'alarm/heartbeat') {
				heartbeat = JSON.parse(msg);
				return;
			}

			if (server_messages.length > 20) server_messages.pop();
...
<div class="flex h-96">
	<div class="flex flex-col justify-between w-50 p-1">
		...
		<span class:bg-green-700={alarmStatus === 'online'} class="p-2 rounded-lg bg-slate-200"
			>Alarm</span
		>
	</div>
