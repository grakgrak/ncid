<script lang="ts">
    ...
    function handleConsider(e:CustomEvent<DndEvent<Task>>, state: TaskState) {
        taskList[state] = e.detail.items;
    }

    function handleFinalize(e:CustomEvent<DndEvent<Task>>, state: TaskState) {
        ...
        <section 
            ...
            <div class="card bg-slate-700 m-2" animate:flip="{{duration: flipDurationMs}}">
                <form class="m-3 card-title" method="POST" action="?/updateState">
                    <span class="w-full decoration-red-500" class:line-through={item.state === TaskState.DONE}>{item.task}</span>
